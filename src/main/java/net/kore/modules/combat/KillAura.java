...
public class KillAura extends Module
{
    ...
    public void onMovePre(final MotionUpdateEvent.Pre event) {
        if ((Kore.scaffold.isToggled() && Scaffold.disableAura.isEnabled()) || !KillAura.DISABLE.hasTimePassed(100L) || !this.isToggled() || (this.onlySword.isEnabled() && (Kore.mc.thePlayer.getHeldItem() == null || !(Kore.mc.thePlayer.getHeldItem().getItem() instanceof ItemSword)))) {
            KillAura.target = null;
            return;
        }
    ...
    public void onMovePost(final MotionUpdateEvent.Post event) {
        ...
        if (KillAura.target != null && Kore.mc.thePlayer.getDistanceToEntity((Entity)KillAura.target) < Math.max(this.rotationRange.getValue(), this.range.getValue()) && this.attacks > 0) {
            ...
            while (this.attacks > 0) {
                ...
                if (Kore.mc.thePlayer.getDistanceToEntity((Entity)KillAura.target) < this.range.getValue() && (RotationUtils.getRotationDifference(RotationUtils.getRotations(KillAura.target), RotationUtils.getLastReportedRotation()) < 15.0 || this.rotationMode.is("None") || Kore.speed.isToggled())) {
